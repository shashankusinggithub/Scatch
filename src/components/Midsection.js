  ...
  return (
    <div className="flex flex-row ">
      ...
      <div className="w-2/5 space-y-2 p-4 ">
        ...
        <input
          className="rounded-md w-full h-10 border-2 border-black"
          type="text"
          name="url"
          placeholder="Enter Sprite/Img URL"
          onChange={HandleChange}
        />
        <input
          className="rounded-md w-full h-10 p-2 border-2 border-black"
          name="name"
          placeholder="Enter Sprite Name"
        />
        <button
          className="bg-slate-300 px-2 rounded-sm border-2 border-black"
          ...
          {props.flow.map((item, index) => {
            return (
              <div className="space-y-2 p-4">
                <img
                  className={`w-20 ${
                    activeSprite === index && "border-4 border-black"
                  } `}
                  src={item.url}
                  onClick={() => {
