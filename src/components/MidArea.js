...
export default function MidArea(props) {
  ...
  return <div
    ref={drop} draggable={false} className={`${props.activeSprite !== props.spriteIndex && 'hidden'} flex-none h-screen relative overflow-y-auto flex flex-col items-start  border-r border-gray-200 text-600 text-2xl font-bold p-5`}>
    {"Drop Blocks to Animate"}    
    
    <Reorder.Group axis={"y" || "x"} values={board} id="midarea" className=" flex-row z-50 py-4 -space-y-2" onReorder={setBoard}>
      {board.map((item) => (
    ...
    <img className="absolute top-6 right-0 h-20 opacity-50 z-0 " src={props.url}/>
  </div>;
}
